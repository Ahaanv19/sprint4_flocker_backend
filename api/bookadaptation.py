...
@bookadaptations.route('/movies/search', methods=['GET']) # run something like http://localhost:6000/movies/search?title=1984 to check in postman (it works)
def search_movie():
...
if __name__ == '__main__': ##run the server on port 6000
    bookadaptations.run(debug=True, port=6000)
